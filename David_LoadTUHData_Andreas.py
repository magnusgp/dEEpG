#### Taken from https://github.com/DavidEnslevNyrnberg/DTU_DL_EEG/blob/master/LoadTUHData_Andreas/loadTUHData.py ####
...
class Load_TUH:
    ...
    def make_dict(self, dataset_path=r"C:\Users\Andreas\Desktop\KID\Fagproject\Data\artifact_dataset"):
        ...
        for ref in os.listdir(
                dataset_path):  # should loop over reference types 01_tcp_ar (avage reference) or 02_tcp_le (link era) or 03_tcp_ar_a(?)
                ...
                for id in os.listdir(ref_path):
                    ...
                    for patient in os.listdir(id_path):
                        ...
                        for session in os.listdir(patient_path):
                            ...
                            EEG_dict.update({EEG_id: {"ref": ref, "id": id, "patient_id": patient, "session": session,
                                                      "path": session_path}})
                            EEG_id += 1
...
def ten_twenty():
    ...
    '''
    return ['FP1', 'FP2', 'F3', 'F4', 'C3', 'C4', 'P3', 'P4', 'O1', 'O2', 'F7', 'F8', 'T3', 'T4', 'T5', 'T6', 'FZ',
            'CZ', 'PZ', 'A1', 'A2']
